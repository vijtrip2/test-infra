...
for CONTROLLER_NAME in $CONTROLLER_NAMES; do
  ...
  if ! make build-controller >/dev/null 2>>/tmp/"$SERVICE_NAME"_failure_logs; then
    ...
    echo "ok."
  COMMAND_OUTPUT=$(cat /tmp/"$SERVICE_NAME"_failure_logs)
    ...
    if [[ -z $ISSUE_NUMBER ]]; then
      ...
      if ! gh issue create -R "$GH_ORG/$GH_ISSUE_REPO" -t "$ISSUE_TITLE" -b "$ISSUE_BODY" . See the latest comment for error output." >/dev/null ; then
        echo ""
  ...
  pushd "$WORKSPACE_DIR/$CONTROLLER_NAME" >/dev/null
    ...
    if [[ -n $PR_NUMBER ]]; then
      ...
      if [[ ! -f /tmp/ack-bot-pr-body ]]; then
        ...
        echo "$COMMIT_MSG" >> /tmp/ack-bot-pr-body
      ...
      if ! gh pr create -R "$GH_ORG/$CONTROLLER_NAME" -t "$COMMIT_MSG" -F /tmp/ack-bot-pr-body -H $PR_SOURCE_GH_BRANCH >/dev/null ; then
        echo ""
